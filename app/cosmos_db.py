...
def get_all_photos(search: str = None, owner_id: str = None):
    if owner_id:
        query = f"SELECT * FROM c WHERE c.owner_id = '{owner_id}' ORDER BY c.created_at DESC"
    elif search:
        term = search.lower()
    ...
    return list(photos_container.query_items(query=query, enable_cross_partition_query=True))
...
def get_comments_for_photo(photo_id: str):
    query = f"SELECT * FROM c WHERE c.photo_id = '{photo_id}' ORDER BY c.created_at ASC"
    return list(comments_container.query_items(query=query, enable_cross_partition_query=True))

# ─── RATING OPERATIONS ───────────────────────────────────────────────────────

def create_or_update_rating(rating_data: dict):
    # Check if user already rated this photo
    query = f"SELECT * FROM c WHERE c.photo_id = '{rating_data['photo_id']}' AND c.user_id = '{rating_data['user_id']}'"
    existing = list(ratings_container.query_items(query=query, enable_cross_partition_query=True))
    if existing:
...
def get_ratings_for_photo(photo_id: str):
    query = f"SELECT * FROM c WHERE c.photo_id = '{photo_id}'"
    return list(ratings_container.query_items(query=query, enable_cross_partition_query=True))

def get_user_rating_for_photo(photo_id: str, user_id: str):
    query = f"SELECT * FROM c WHERE c.photo_id = '{photo_id}' AND c.user_id = '{user_id}'"
    items = list(ratings_container.query_items(query=query, enable_cross_partition_query=True))
    return items[0] if items else None